...
tags: ["hugo", "веб-разработка", "блог"]
categories: ["Технологии", "Туториалы"]
author: "Your name"
...
```
## Отрегулировать, сколько первых строк поста попадает на главную страницу с заголовками

1️⃣ Ручной контроль (рекомендую)
Добавь <!--more--> в посте — всё до этой метки попадёт в summary

2️⃣ Автоматическая длина (количество слов)
В hugo.toml добавь:
```
...
summaryLength = 30  # ← количество слов в summary (по умолчанию 70)

[params]
  author = "Ваше имя"
  description = "Блог о..."
```
3️⃣ Кастомное описание для каждого поста
Добавь поле summary в frontmatter:
```
---
title: "Мой пост"
date: 2024-03-18T12:00:00+03:00
draft: false
summary: "Это кастомное описание поста, которое точно появится на главной."
---
...
## Относительные пути в Markdown

В Hugo поддерживаются относительные пути для ссылок на файлы в той же папке.

Shortcode relref (рекомендуемый метод):

```
<!-- Ссылка на готовую страницу (рекомендуется) -->
[Читать другой пост]({{< relref "perfect-setup.md" >}})
Или с полным путём: 
[туториал]({{< relref "/posts/hello-world.md" >}}).
```

Hugo проверяет существование файла при сборке. Автоматически генерирует правильный URL. Работает при изменении структуры папок. Выдаёт ошибку, если файл не найден.
